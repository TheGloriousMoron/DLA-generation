...
{
    return grid->array[pos[0]][pos[1]] == grid->adjecent_state;
}
...
void set_particle(grid_t *grid, int32_t *pos)
{
    ...
    {
        grid->array[pos[0] - 1][pos[1]] = grid->adjecent_state;
    }
    ...
    {
        grid->array[pos[0]][pos[1] - 1] = grid->adjecent_state;
    }
...
void grid_run_simulation(grid_t *grid)
{
    ...
    while (grid->current_particles < grid->max_particles)
    {
        ...
        while (particle_iterations < grid->iteration_max)
        {
            ...
            if (valid_move)
            {
                ...
                if (check_particle_adjecent(grid, pos))
                {
                    ...
                    break; // Exit the iteration loop as particle is placed
                }
            }
        ...
        // If max iterations are reached, place the particle at its current position
